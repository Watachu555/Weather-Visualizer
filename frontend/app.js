document.addEventListener("DOMContentLoaded", () => {
  ...
  const API_BASE = window.location.hostname.includes("localhost")
    ? "http://localhost:8080"
    : "https://weather-visualizer-1.onrender.com";
  ...
  weatherButton.addEventListener("click", () => {
    ...
    fetch(`${API_BASE}/api/weather?city=${encodeURIComponent(userLocation)}`)
      ...
      .then((data) => {
        if (data.error) {
          ...
          const { temp_c, condition, is_day, humidity } = data.current;
          ...
          // 🎨 Background gradient + particle type
